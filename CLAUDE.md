...
- Chat ([backend/src/routes/chat.js](file:///d:/xiaoai-tongxue/lvs-trtc/backend/src/routes/chat.js) + [backend/src/lib/chat.js](file:///d:/xiaoai-tongxue/lvs-trtc/backend/src/lib/chat.js)): direct conversations are E2E-encrypted client-side (ECDH P-256 + AES-GCM; server stores public keys and ciphertext only, rejects plaintext in direct convs). Each user gets one lazy-created `system` conversation: server pushes plaintext notifications into it via `sendSystemMessage()` (review results, report resolution, takedown, ban, new comments), and user messages sent there become admin feedback (`admin.feedback.created` event, `GET /api/admin/feedback`, reply via `POST /api/admin/feedback/:userId/reply`). New-message fan-out uses `chat.message.new` on the personal WS channel.
